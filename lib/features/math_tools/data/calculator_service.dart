// lib/features/math_tools/data/calculator_service.dart
import '../domain/big_decimal.dart';
import '../domain/calculation_result.dart';

/// Service Logika Kalkulator Presisi Tinggi & Evaluasi Ekspresi Matematika.
/// 
/// Memenuhi FR-T1-01 (Kalkulator Presisi):
/// - Mendukung presisi desimal tak terbatas (Arbitrary-Precision BigDecimal).
/// - Mendukung angka negatif & operator unary minus (contoh: 124240190.124240191 * -124240192).
/// - Mendukung ekspresi matematika kompleks dengan tanda kurung dan urutan presedensi (*, /, +, -).
class KalkulatorService {
  ...
  void _prosesSingleItem(String item) {
    ...
    if (isOp) {
      if (_tokens.isEmpty) {
        ...
        if (_isOperator(lastToken) || lastToken == '(') {
          if (item == '-' || item == '+') {
            _tokens.add(item);
          } else {
      ...
      if (_tokens.isNotEmpty && !_isOperator(_tokens.last) && _tokens.last != '(' && _tokens.last != '+' && _tokens.last != '-') {
        _tokens.add('+');
      }
  ...
  static BigDecimal _evaluateTokens(List<String> tokens) {
    ...
    if (values.isEmpty) throw const FormatException('Format ekspresi tidak lengkap.');
